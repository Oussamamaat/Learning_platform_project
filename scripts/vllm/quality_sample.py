...
def main() -> int:
    ...
    for case in cases:
        ...
        md_lines.append(f"> {row.get('ollama_answer') or ('ERROR: ' + str(row.get('ollama_error')))}\n")
        md_lines.append("**vLLM (AWQ-INT4):**\n")
        md_lines.append(f"> {row.get('vllm_answer') or ('ERROR: ' + str(row.get('vllm_error')))}\n")
        md_lines.append("---\n")
